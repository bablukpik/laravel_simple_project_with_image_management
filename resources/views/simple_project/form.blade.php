...
<html lang="en">
  ...
  <body>
    ...
    @if (Session::has('unsuccess'))
      <span style="color:green"> {!! Session::get('unsuccess') !!} </span>
    @endif  
  <a class="btn btn-danger" href="http://localhost/blog/public/showalldata">View data</a>

 		{{ Form::open(array('action' => 'SimpleController@store', 'files' => true)) }}
          ...
          @if ($errors->has('form_name'))
           <span style="color:red">{!! $errors->first('form_name', '') !!}</span>
          @endif
			  </div>
			  <div class="form-group">
			    <label for="exampleInputEmail1">E-mail</label>
			    <input type="email" name="form_email" class="form-control" id="exampleInputEmail1" placeholder="E-mail">
          @if ($errors->has('form_email'))
           {!! $errors->first('form_email', '<span style="color:red">Rong email</span>') !!}
          @endif
			  </div>
			  <div class="form-group">
			    <label for="exampleInputPassword1">Password</label>
			    <input type="password" name="form_password" class="form-control" id="exampleInputPassword1" placeholder="Password">
          @if ($errors->has('form_password'))
